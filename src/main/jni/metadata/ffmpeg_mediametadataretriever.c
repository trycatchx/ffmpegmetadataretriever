...
int interruptCallback(void *p) {
    ...
    if (r->lasttime > 0) {
        if ((time(NULL) - r->lasttime > r->timeout)
            || (r->interrupt == 1)) {
            ALOGV("test call:%p - %d", r, r->interrupt);
            return 1;
        }
    }
...
int set_data_source_l(State **ps, const char *path, Runner *runner) {
    int audio_index = -1;
    int video_index = -1;
    int i;
...
//    av_log_set_callback(log_callback_null);
    ...
    state->pFormatCtx->interrupt_callback.opaque = (void *) runner;
    runner->lasttime = time(NULL);


    if (runner->interrupt == 1
        || avformat_open_input(&state->pFormatCtx, path, NULL, &options) != 0) {

        if (state->pFormatCtx) {
            state->pFormatCtx->interrupt_callback.callback = NULL;
            state->pFormatCtx->interrupt_callback.opaque = NULL;
    ...
    ALOGV("test P Runner11111 %p-%p", runner, state);

    if (runner->interrupt == 1
        || avformat_find_stream_info(state->pFormatCtx, NULL) < 0) {
        if (state->pFormatCtx) {
            state->pFormatCtx->interrupt_callback.callback = NULL;
            state->pFormatCtx->interrupt_callback.opaque = NULL;
            avformat_close_input(&state->pFormatCtx);
...
int set_data_source_uri(State **ps, const char *path, const char *headers, Runner *runner) {
    return set_data_source_l(ps, path, runner);
}

void initPre(State **ps, const char *path, const char *headers) {
    State *state = *ps;
...
int set_data_source_fd(State **ps, int fd, int64_t offset, int64_t length) {
    ...
    *ps = state;
    ...
    return set_data_source_l(ps, path, NULL);
}
...
int get_embedded_picture(State **ps, AVPacket *pkt,Runner *runner) {
...
void convert_image(State *state, AVCodecContext *pCodecCtx, AVFrame *pFrame, AVPacket *avpkt,
    ...
    *got_packet_ptr = 0;
    ...
    if ((width != -1 && height != -1) || state->sws_ctx == NULL) {
...
int get_frame_at_time(State **ps, int64_t timeUs, int option, AVPacket *pkt,Runner *runner) {
    return get_scaled_frame_at_time(ps, timeUs, option, pkt, -1, -1, runner);
}
...
int get_scaled_frame_at_time(State **ps, int64_t timeUs, int option, AVPacket *pkt, int width,
    ...
    if (timeUs != -1) {
        ...
        ALOGV("test http @ seek timepre  %lld",timeUs);
        ALOGV("test http @ seek timepre1  %lld",seek_time);
        int flags = 0;
        int ret = -1;
    ...
    decode_frame(state, pkt, &got_packet, desired_frame_number, width, height,runner);
